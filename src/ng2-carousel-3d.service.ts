...
export class Ng2Carousel3dService {
...
setSlides() {
    ...
    var rightOut = this.leftOutSlide = (this.currentIndex - num),
        leftOut = this.rightOutSlide = ((this.total - this.currentIndex - num) <= 0) ? (-(this.total - this.currentIndex - num)) : (this.currentIndex + num);
...
getVisibleSlidesIndex() {
    ...
    let visibleSlidesArr = Object.keys(visibleSlidesObj).map(key=>visibleSlidesObj[key])
